...
/**
 * @brief 使用行列式扫描方法 这是清翔的使用方法
 *
 * @return u8
 */
u8 key_matrix_ranks_scan2()
{
	u8 hangxian, liexian;
	// 0000 1111  将行线全赋0 列线全赋1
	KEY_MATRIX_PORT = 0x0f;
	//判断是否有按键按下
	//如果有按键按下的话 列线肯定会有一位变成0
	if ((KEY_MATRIX_PORT & 0x0f) != 0x0f)
	{
		//消抖
		delay_10us(1000);
		//再次判断
		...
		{
			//将列线保存起来
			liexian = KEY_MATRIX_PORT & 0x0f;
			KEY_MATRIX_PORT = liexian | 0xf0;
			hangxian = KEY_MATRIX_PORT & 0xf0;
			//松手检测
			while ((KEY_MATRIX_PORT & 0xf0) != 0xf0)
				;
			return hangxian + hangxian;
		}
	}
}
